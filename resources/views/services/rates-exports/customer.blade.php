...
@section('title', trans('rate.export_customer_rates'))
...
<table>
    <thead>
        <tr><th colspan="2"><h3>{{ trans('rate.export_customer_rates') }}</h3></th></tr>
...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td>{{ 1 + $key }}</td>
            <td>{{ $rate->customer->account_no }}</td>
            <td>{{ $rate->customer->code }}</td>
