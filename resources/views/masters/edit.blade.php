    ...
    <div class="col-md-4">
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                {!! FormField::text('name',['label'=> trans('master.name')]) !!}
                {!! FormField::textarea('description',['label'=> trans('master.description')]) !!}
            </div>

            <div class="panel-footer">
                {!! Form::submit(trans('master.update'), ['class'=>'btn btn-primary']) !!}
                {!! link_to_route('masters.show', trans('master.show'), [$master->id], ['class' => 'btn btn-info']) !!}
                {!! link_to_route('masters.index', trans('master.back_to_index'), [], ['class' => 'btn btn-default']) !!}
                {!! link_to_route('masters.delete', trans('master.delete'), [$master->id], ['class'=>'btn btn-danger pull-right']) !!}
            </div>
        </div>
        {!! Form::close() !!}
    </div>
