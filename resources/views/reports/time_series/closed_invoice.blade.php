...
@section('title', __('report.time_series.time_series').': '.__('report.time_series.closed_invoice'))

@section('content')
<div class="well well-sm">
    <h3 class="pull-left" style="margin-top: 4px; margin-bottom: 0px;">{{ __('report.time_series.closed_invoice') }}</h3>
    ...
    {!! link_to_route('reports.time_series.closed_invoice', __('report.this_year'), [], ['class' => 'btn btn-default']) !!}
    {!! Form::close() !!}
</div>
    ...
    <table class="table table-condensed table-hover" style="font-size:11px">
        ...
        <tbody>
            <tr>
                ...
                <td>UMUM</td>
                @php
                    $monthNonCustomerRecapTotal = $invoiceTimeSeriesRecap->filter(function ($item) use ($paymentTypeId) {
                        return $item->customer_id == null && $item->type_id == $paymentTypeId;
                    })->sum('amount');
                @endphp
                @foreach (getMonths() as $monthNum => $month)
                    @php
                        $monthlyInvoiceTimeSeriesRecap = $invoiceTimeSeriesRecap->filter(function ($item) use ($monthNum, $paymentTypeId) {
                            return $item->customer_id == null && $item->month == (int) $monthNum && $item->type_id == $paymentTypeId;
                        })->first();
                        $billRecap = $monthlyInvoiceTimeSeriesRecap ? $monthlyInvoiceTimeSeriesRecap->amount : 0;
                    @endphp
            ...
            @php
                $monthCustomerRecapTotal = $invoiceTimeSeriesRecap->filter(function ($item) use ($customer, $paymentTypeId) {
                    return $item->customer_id == $customer->id && $item->type_id == $paymentTypeId;
                })->sum('amount');
            @endphp
            ...
            <tr>
                ...
                @foreach (getMonths() as $monthNum => $month)
                    @php
                        $monthlyInvoiceTimeSeriesRecap = $invoiceTimeSeriesRecap->filter(function ($item) use ($customer, $monthNum, $paymentTypeId) {
                            return $item->customer_id == $customer->id && $item->month == (int) $monthNum && $item->type_id == $paymentTypeId;
                        })->first();
                        $billRecap = $monthlyInvoiceTimeSeriesRecap ? $monthlyInvoiceTimeSeriesRecap->amount : 0;
                    @endphp
                    <td class="text-right">{{ formatNo($billRecap) }}</td>
