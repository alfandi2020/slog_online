...
@section('title', trans('manifest.problems.list'))

@section('content')
<div class="pull-right">
    {!! html_link_to_route('manifests.problems.create', trans('manifest.problems.create'), [], ['class' => 'btn btn-success']) !!}
</div>
<h2 class="page-header">{{ trans('manifest.problems.list') }}</h2>
...
<table class="table table-condensed">
    <thead>
        ...
        <th class="text-right">{{ trans('manifest.bill_total') }}</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                {{ link_to_route('manifests.problems.show', $manifest->number,[$manifest->number], [
            ...
            <td class="text-center">{{ $manifest->receipts()->count() }}</td>
            <td class="text-right">{{ formatRp($manifest->receipts->sum('bill_amount')) }}</td>
            <td>{!! $manifest->present()->statusLabel !!}</td>
            <td>
                {!! html_link_to_route('manifests.problems.show', '', [$manifest->number],[
                    'icon' => 'search',
                    'title' => 'Lihat detail manifest ' . $manifest->number,
                    'class' => 'btn btn-info btn-xs',
                ]) !!}
                @if($manifest->receipts_count)
                {!! html_link_to_route('manifests.problems.pdf', '', [$manifest->number], [
                    'icon' => 'print',
