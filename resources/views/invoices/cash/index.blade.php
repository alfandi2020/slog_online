...
@section('title', trans('cash_invoice.list'))

@section('content')
<div class="pull-right">
    @can('create', new App\Entities\Invoices\Cash)
    {{ link_to_route('invoices.cash.create', trans('cash_invoice.create'), [], ['class' => 'btn btn-success btn-sm','id' => 'create_cash_invoice']) }}
    @endif
</div>

@include('invoices.partials.cash-index-nav-tabs')

<div class="panel panel-default table-responsive">
    <div class="panel-heading">
        <h3 class="panel-title">
            {{ trans('cash_invoice.list_title', ['status' => request('status'), 'count' => $invoices->total()]) }}
        </h3>
    </div>
    <table class="table table-condensed">
        <thead>
            <th>{{ trans('app.table_no') }}</th>
            <th>{{ trans('cash_invoice.number') }}</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    {!! link_to_route('invoices.cash.show', 'Detail',[$invoice->id],['class'=>'btn btn-info btn-xs']) !!}
                </td>
            </tr>
            <?php
            // $sumCount += $count;
            ...
            @empty
            <tr>
                <td colspan="9">{{ trans('cash_invoice.empty') }}</td>
            </tr>
            @endforelse
        </tbody>
        <?php /*
