@if (Request::get('action') == 'delete' && Request::has('file_name'))
    <div class="panel panel-danger">
        ...
        <div class="panel-footer">
            <a href="{{ route('admin.backups.index') }}" class="btn btn-default">{{ trans('backup.cancel_delete') }}</a>
            <form action="{{ route('admin.backups.destroy', Request::get('file_name')) }}" method="post" class="pull-right">
                {{ method_field('delete') }}
                {{ csrf_field() }}
...
@if (Request::get('action') == 'restore' && Request::has('file_name'))
    <div class="panel panel-warning">
        ...
        <div class="panel-footer">
            <a href="{{ route('admin.backups.index') }}" class="btn btn-default">{{ trans('backup.cancel_restore') }}</a>
            <form action="{{ route('admin.backups.restore', Request::get('file_name')) }}"
                method="post"
                class="pull-right"
